...
import {getScienceResearchFunctions} from "@/api/custom/auxiliaryWork";
...
export default {
  ...
  async mounted() {
    let functionResp = await getScienceResearchFunctions();
...
<div class="flex flex-col items-center w-full h-full overflow-hidden relative">
  <FunctionTitle title="科研辅助" class-name="mt-[160px]"/> 
  <div class="w-full flex justify-center gap-[20px] mt-[40px]">
  <div class="w-[700px] flex flex-wrap gap-2 2xl:gap-4">
    <div class="w-[342px] h-[128px]" v-for="functionItem in functions">
      <FunctionShortcut
