    ...
    <el-dialog :key="`dialog-${fileLoading}`" class="upload-file-dialog" :class="{ 'right-bottom': fileLoading, 'dialog-less': dialogLess }" :visible="show" :show-close="false"
        ...
        <div class="w-full p-[16px]">
            <div class="w-full flex flex-col items-center border-[#C9CDD4] border-[1px] px-[8px] rounded-[8px] overflow-hidden relative"
                ...
                <FileUpload ref="fileUpload" v-model="uploadFileList" :is-show-tip="false" :auto-upload="false" :need-load-list="needLoadList"
                    :class="['w-full', 'h-full', 'flex', 'flex-col', 'max-h-[400px]', { 'isUpload': uploadFileList.length }]" :limit="limit"
                    :file-size="fileSize" :file-type="fileType" :url="url" :data="data" :handleLoadListComplate="listComplate"
                    ...
                    <template v-else-if="!uploadFileList.length" class="w-full">
                        <div class="w-full flex flex-col justify-center items-center py-[24px]"
                            ...
                            <div class="text-[#C9CDD4] px-[32px] text-[12px] leading-[18px]">
                                可上传{{ fileType.join(',') }}文件,每个文件不超过{{ fileSize }}MB,每次上传数量不超过{{ limit }}个</div>
                        </div>
                    </template>
                    <div v-else>
                        <span
...
export default {
    ...
    props: {
        ...
        fileType: { type: Array, default: () => ['pdf', 'docx', 'doc', 'txt'] },
        url: { type: String, default: '/attachment/chat/upload' },
        data: { type: Object, default: () => ({}) },
        needLoadList: { type: Boolean, default: false },
    },
    data() {
        return {
            uploadFileList: [],
    ...
    methods: {
        ...
        listComplate(fileList) {
            this.$emit('loadListComplate', fileList)
        },
    },
}
</script>
